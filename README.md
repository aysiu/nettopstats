...
It basically runs the `nettop -L 1` command every 20 minutes and saves the output to one .json file per day.
...
Probably not that intensive. Originally, I had it run every few seconds, but that turned out to give widely inaccurate data, and even that didn't seem too resource intensive. Now that the interval is set to every 20 minutes, I highly doubt you'll see any impact at all. No promises, though.
